    ...
    for idx, (images, labels, metadata) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast():
            if args.method == 'ce':
                ...
                # mix_images, labels_a, labels_b, lam, index = model(images, y=labels, patch_mix=True, time_domain=args.time_domain)
                mix_images, labels_a, labels_b, lam, index_combinatnions = model(images, y=labels, patch_mix=True, time_domain=args.time_domain)
                proj2 = projector(mix_images)
                loss += args.alpha * criterion[1](proj1, proj2, labels, labels_b, lam, index_combinatnions, args)
