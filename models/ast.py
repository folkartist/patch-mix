...
class ASTModel(nn.Module):
    ...
    def patch_mix(self, image, target, time_domain=False, hw_num_patch=None):
        ...
        if self.mix_beta > 0:
            ...
            lam =np.random.dirichlet(self.mix_beta*np.ones(mix_number))

        else:
            # lam = 1
            lam=np.ones(mix_number)

        batch_size, num_patch, dim = image.size()
        device = image.device

        index = torch.randperm(batch_size).to(device)
        index = np.arange(batch_size).to(device)
        index_combinations = torch.tensor(list(combinations(index, mix_number)))
        if not time_domain:
            # num_mask = int(num_patch * (1. - lam))
            split_points = np.cumsum([int(lam_i * len(num_patch)) for lam_i in lam[:-1]])
            split_points = np.concatenate(([0], split_points, [-1]))
            # mask = torch.randperm(num_patch)[:num_mask].to(device)
            mask=[torch.randperm(num_patch)[split_points[i]:split_points[i+1] ].to(device) for i in range(len(split_points)-1)]
            index = torch.randperm(batch_size)
            index = np.arange(batch_size)
            index_combinations=random.sample(list(combinations(index, 3)), batch_size)
            index_combinations = torch.tensor(index_combinations)
            ...
            num_patch=1024
            masks=[torch.randperm(num_patch)[split_points[i]:split_points[i+1] ] for i in range(len(split_points)-1)]
            for comb in index_combinations: 
                for mask in masks:
                    for i in range(1,len(comb)):
                        image[comb[0],mask,:]=image[comb[i],mask,:]
        ...
        return image, y_a, y_b, lam, index_combinations
    ...
    def forward(self, x, y=None, patch_mix=False, time_domain=False):
        ...
        if patch_mix:
            ...
            x, y_a, y_b, lam, index_combinations= self.patch_mix(x, y, time_domain=time_domain, hw_num_patch=[h_patch, w_patch])
        ...
        if not patch_mix:
            ...
            return x, y_a, y_b, lam, index_combinations